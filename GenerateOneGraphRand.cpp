...
vector<int> Remove(vector<int> vect,int value)
{
    for(int i=0;i<vect.size();i++)
    {
        if(value==vect[i])
...
//Generuje macierz sąsiedztwa spójnego grafu 
//------------------------------------------
int main()
{
    const string fileInputName = "Deegres.json"; 
    const string fileOutputName = "Graphs6.json";
    ...
    vector<vector<int>> unUsedCombs;            //Indeksy nie użytych kombinacji 
        ...
        {   
            ...
            if(nextC)
            {
                ...
                if(AllCombinChecked)        //Czy wszystkie kombinacje zostały sprawdzone
                {
                    ...
                    {
                        unUsedCombs[x].push_back(i);
                    }
                    combinationIndex[x]=rand()%unUsedCombs[x].size();           //Losowanie kombinacji do sprawdzenia
                    combinationIndex[x]=unUsedCombs[x][combinationIndex[x]];
                    unUsedCombs[x] = Remove(unUsedCombs[x],combinationIndex[x]);
                }
